 ...
 *   based on input from faceect
 *
 * Usage:
 *	var hp = new headtrackr.headposition.Tracker(faceect, 640, 480);
 *	var currentPosition = hp.track(faceect);
 *
 * @author auduno / github.com/auduno
 *
 * Parameters to Tracker() are:
 *	faceect : a generic object with attributes x, y, width, height, angle
 ...
 *	 headtrackr.headposition.Tracker(faceect, 640, 480, {fov : 60})
 *
 * Optional parameters:
 ...
 * @constructor
 */
 function FieldOfViewCalculator(face, camwidth, camheight, params) {
	...
	if (params.edgecorrection === undefined) {
		...
		edgecorrection = params.edgecorrection;
	}
	
	const camheight_cam = camheight;
	const camwidth_cam = camwidth;
	
	const head_width_cm = 16;
	const head_height_cm = 19;
	
	// angle between side of face and diagonal across
	const head_small_angle = Math.atan(head_width_cm/head_height_cm);
	
	const head_diag_cm = Math.sqrt((head_width_cm*head_width_cm)+(head_height_cm*head_height_cm)); // diagonal of face in real space
	
	const sin_hsa = Math.sin(head_small_angle); //precalculated sine
	const cos_hsa = Math.cos(head_small_angle); //precalculated cosine
	const tan_hsa = Math.tan(head_small_angle); //precalculated tan
	
	// estimate horizontal field of view of camera
	const init_width_cam = face.width;
	const init_height_cam = face.height;
	let head_diag_cam = Math.sqrt((init_width_cam*init_width_cam)+(init_height_cam*init_height_cam));
	let fov_width;
	if (params.fov === undefined) {
		...
		const head_width_cam = sin_hsa * head_diag_cam;
		const camwidth_at_default_face_cm = (camwidth_cam/head_width_cam) * head_width_cm;
		// we assume user is sitting around 60 cm from camera (normal distance on a laptop)
		let distance_to_screen;
		...
		fov_width = Math.atan((camwidth_at_default_face_cm/2)/distance_to_screen) * 2;
	} else {
		fov_width = params.fov * Math.PI/180;
	}
	
	// precalculate ratio between camwidth and distance
	const tan_fov_width = 2 * Math.tan(fov_width/2);
	
	let x, y, z; // holds current position of head (in cms from center of screen)
	...
	this.track = (face) => {
		...
		let fx = face.x; 
		let fy = face.y; 
		let originalWeight, estimateWeight;
		...
		if (edgecorrection) {
			...
			const rightDistance = camwidth_cam-(fx+(w/2));
			const topDistance = fy-(h/2);
			const bottomDistance = camheight_cam-(fy+(h/2));
			
			const onVerticalEdge = (leftDistance < margin || rightDistance < margin);
			const onHorizontalEdge = (topDistance < margin || bottomDistance < margin);
		...
		return {x,y,z, fov: fov_width * 180/Math.PI};
	}
};
